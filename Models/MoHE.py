...
MoHE Modules
https://github.com/evortigosa/mohe_forecast
"""
...
class MoEFeedForward(nn.Module):
    """
    The Sparse Mixture-of-Experts (MoE) module for heterogeneous experts (MoHE). Delegate the modeling of
...
class QKRoPEv2(nn.Module):
    """
    The Rotary Positional Encoding (RoPE) functions for Query and Key embeddings.
    This version precomputes rotation factors (in real trigonometric) and applies them to the
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, multi_modal, depth, d_model=384, block_size=672, n_heads=12, n_kv_heads=6, d_ff=768,
        ...
        self.s_att= MultiHeadedAttention(
            ...
            use_qk_norm, headwise_attn_gate
        )
...
class TransformerModel(nn.Module):
    ...
    !!! some changes are made for the pollutant forecasting tasks
    """
    ...
    def __init__(self, multi_modal, is_causal, n_layer=8, d_model=384, block_size=672, n_heads=12, n_kv_heads=6,
                 ...
                 seq_len=336, pred_len=168, n_vars=3, cross_vars=26) -> None:
        ...
        self.embedding = nn.Linear(self.n_vars, d_model)
        self.cross_embedding = nn.Linear(self.cross_vars, d_model)
        ...
        self.final_norm= self.transformer[-1].get_norm(norm_type, n_vars, init_alpha=0.2)
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.model = TransformerModel(
            multi_modal=configs.multi_modal,
            is_causal = configs.is_causal,
            n_layer = configs.n_layer,
            d_model = configs.d_model,
            block_size = configs.block_size,
            n_heads = configs.n_heads,
            n_kv_heads = configs.n_kv_heads,
            d_ff = configs.d_ff,
            dropout = configs.dropout,
            drop_path = configs.drop_path,
            norm_type = configs.norm_type,
            diff_attn = configs.diff_attn,
            ffn_type = configs.ffn_type,
            glu = configs.glu,
            n_experts = configs.n_experts,
            top_k_experts = configs.top_k_experts,
            experts_type = configs.experts_type,
            exp_route_dropout = configs.exp_route_dropout,
            exp_route_temperature = configs.exp_route_temperature,
            bias = configs.bias,
            rope_theta = configs.rope_theta,
            use_qk_norm = configs.use_qk_norm,
            headwise_attn_gate = configs.headwise_attn_gate,
            c_att_mode = configs.c_att_mode,
...
if __name__ == "__main__":
    model = TransformerModel(
        multi_modal=False, is_causal=False, n_layer=3, d_model=384, block_size=672, n_heads=6, n_kv_heads=3,
        d_ff=768, dropout=0.2, drop_path=0.3, norm_type='rms', diff_attn=False, ffn_type='dwconv', glu=False,
        n_experts=8, top_k_experts=2, experts_type='fan', exp_route_dropout=0.1, exp_route_temperature=1.0,
        bias=False, rope_theta=10000.0, use_qk_norm=False, headwise_attn_gate=False, c_att_mode='full',
        seq_len=336, pred_len=168, n_vars=3, cross_vars=26
    )

    x = torch.rand((64, 336, 3))
    y, _ = model(x, None, None)
    print(y.shape)
